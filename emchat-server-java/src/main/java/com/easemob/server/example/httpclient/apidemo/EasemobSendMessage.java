...
public class EasemobSendMessage {
	...
	private static Client getClient(boolean https) throws KeyManagementException, NoSuchAlgorithmException {
		...
		if (https) {
			/*
			 * HostnameVerifier hv = new HostnameVerifier() { // @Override // public boolean verify(String hostname,
			 * SSLSession session) { // System.out.println("Warning: URL Host: " + hostname + " vs. " +
